...
} from "../../reports/service/notifications.services.js";
import {
  deleteService,
  getServiceById,
} from "../../services/services/service.service.js";
  ...
  try {
    ...
    try {
      await createNotificationForPermission(
        "READ_DENUNCIAS",
        `Se ha creado una nueva denuncia, referencia: ${referenceAtribute}: ${idReference}`
      );
    } catch (error) {
      console.log(
        "error creando notificacion para un los usuarios moderadores de una nueva denuncia: ",
        error
      );
    }
...
export async function aproveComplaint(validateComplaintDto) {
    ...
    switch (complaint.category) {
      case "SERVICE":
        ...
        try {
          await createNotificationForUser(
            complaint.idUserReported,
            `Se ha eliminado tu servicio por que no cumple con nuestros terminos y condiciones.`
          );
        } catch (error) {
          console.log(
            "error creando notificacion para un servicio eliminado por denuncias: ",
            error
          );
        }
        break;
      case "SEARCH":
        ...
        try {
          ...
          console.log(
            "error creando notificacion para una busqueda eliminada por denuncias: ",
            error
          );
        }
        break;
      case "ADOPTION":
        ...
        try {
          ...
          console.log(
            "error creando notificacion para una adopcion eliminada por denuncias: ",
            error
          );
        }
        break;
  ...
  try {
    ...
    if (userComplaints.length >= 3) {
      ...
      const bloquedUserState = (await getUserStateByName("BLOQUEADO"))[0]
        .dataValues;
      console.log(bloquedUserState);
      await updateUser(complaint.idUserReported, {
        blocked: true,
        blockedReason: "Maximo de denuncias superado",
      });
      await changeStateUser(
        complaint.idUserReported,
        bloquedUserState.idUserState,
        validateComplaintDto.author.idUser
      );
      try {
        await createNotificationForUser(
          complaint.idUserReported,
        ...
        console.log(
          "error creando notificacion para un usuario bloqueado: ",
          error
        );
      }
    }
...
export async function deleteComplaint(complaint) {
  try {
    ...
    try {
      await createNotificationForUser(
        complaint.idUserReporting,
        `La denuncia que levantaste, fue analizada y rechazada por nuestro equipo, si crees que fue un error comunicate a projectapplicationbuddy@gmail.com. Seguí contribuyendo a mejorar la app!`
      );
    } catch (error) {
      console.log(
        "error creando notificacion para una denuncia eliminada: ",
        error
      );
    }
    return;
